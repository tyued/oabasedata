    ...
    <div id="maindiv" @click="closeDropdown" @keyup.esc="closeDropdown">
        ...
        <div class="col-sm-12">
            ...
            <table class="table table-bordered table-hover table-condensed table-striped vue-table">
                ...
                <tbody>
                     <tr v-for="(entry,index) in filteredValuesSorted " :key="index" track-by="entry" @click="rowClickHandler($event, entry)">
                    <!-- <tr v-for="entry in filteredValuesSorted " track-by="entry" @click="rowClickHandler($event, entry)"> -->
                        <td v-for="column in displayColsVisible" track-by="column"
                            ...
                            <span v-if="column.renderfunction!==false">
                                <button type="button" class="btn btn-sm btn-primary"  @click="saveClick(entry)"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span></button>
                            </span>
...
<script>
    ...
    import {ImportScore} from 'api/project/projectDetail/index'
    ...
    var valueFieldSection = {
      template: '<span v-if="!enabled" @click="toggleInput($event)" class="editableField">{{this.entry[this.columnname]}}</span>'+
          '<div v-else-if="enabled" class="input-group editInput">'+
          '  <input type="text" class="form-control" v-focus v-model="datavalue" @blur="saveThis" @keyup.esc="cancelThis" id="nowInput">'+
      ...
      directives: {
        focus: {
            ...
            inserted: function (el) {
                el.focus()
                // el.parentNode.parentNode.parentNode.style.backgroundColor="lightblue"
            }
        }
    },
    //   watch:{  
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                bjmc: localStorage.getItem("bjmc"),       //班级名称
                filteredSize: 0,
                filterKey: "",
                sortKey: [],
                sortOrders: {},
        ...
        methods: {
            saveClick:function(saveDate){   //上传成绩

                let _this = this
                setTimeout(function() {
                    // this.editNum=1;
                    // console.log(saveDate["姓名"],saveDate["成绩"]);
                    let xh=saveDate["学号"];
                    let xm=saveDate["姓名"];
                    let xb='';
                    let kszt='';
                    ...
                    if(saveDate["考试状态"]=='正常'){
                        kszt=0;
                    }else if(saveDate["考试状态"]=='补考'){
                        kszt=1; 
                    }else if(saveDate["考试状态"]=='缺考'){
                        kszt=2; 
                    }
                    // console.log(kszt);
                    ImportScore([{
