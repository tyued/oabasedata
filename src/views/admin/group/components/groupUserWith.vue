        ...
        create(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    if (this.members.length > 0) this.form.members = this.members.join();
                    modifyUsers(this.groupId,this.form).then(() => {
                        this.$emit('closeUserWithDialog');
                        this.$notify({
                            title: '成功',
                            message: '创建成功',
